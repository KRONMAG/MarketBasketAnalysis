# <a id="MarketBasketAnalysis_Analysis_TomitaAlgorithm"></a> Class TomitaAlgorithm
...
### <a id="MarketBasketAnalysis_Analysis_TomitaAlgorithm_Find__1_System_Collections_Generic_IReadOnlyDictionary___0_System_Collections_Generic_HashSet___0___System_Int32_System_Int32_System_Threading_CancellationToken_"></a> Find<TVertex\>\(IReadOnlyDictionary<TVertex, HashSet<TVertex\>\>, int, int, CancellationToken\)

Finds all maximal cliques in a graph represented by an adjacency list.

```csharp
public IEnumerable<MaximalClique<TVertex>> Find<TVertex>(IReadOnlyDictionary<TVertex, HashSet<TVertex>> adjacencyList, int minCliqueSize, int maxCliqueSize, CancellationToken token = default) where TVertex : struct
```
